...
type AttackState struct {
	requestCounter  atomic.Uint64 // Gunakan atomic untuk counter yang aman antar-goroutine
...
func getRandomReferer(state *AttackState) string {
	...
	// Jika tidak cocok atau tidak bisa diparsing, tambahkan string acak saja
	return ref + buildRandomString(rand.Intn(randomStringMax-randomStringMin+1)+randomStringMin)
}
...
func worker(state *AttackState, wg *sync.WaitGroup) {
	...
	for {
		...
		state.requestCounter.Add(1) // Increment counter untuk setiap upaya request
